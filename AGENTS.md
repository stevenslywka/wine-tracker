...
- Latest production work noted in this guide: mobile Wine Detail polish for Add bottles, Manage bottles, Receive Shipment, Drink History rows, and collapsed section previews, pushed to GitHub `main` after local verification.
...
- Mobile Wine Detail uses a compact `Bottles` stock-control panel: dotted location summary, tinted two-column location stock cards with top-right manage (`...`) and a `- / count / +` stepper row, incoming `Receive Shipment`, compact Drink/Add/Manage sheets, and a collapsed Drink History tasting-journal section with tappable rows for edit/delete.
...
- Collapsed sections: Wine details preview shows Region and Varietal only; Purchase preview is left-aligned.
