...
- Mobile Select mode has bulk Edit: tap `Select`, tap cards or `All`, tap `Edit`, then choose Location, Status, Sticker, Source, or Order Date. These actions post selected IDs to `/wines/bulk-edit`. Location marks selected wines Available and assigns the chosen location. Do not treat this as inventory-history or quantity-drinking behavior.
